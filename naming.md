...
1. Use a single `#` at the start of the comment line.
2. Separate each field with a comma and a space.
3. Use all caps for field names, followed by a colon and a space.
4. If a field is unknown or not applicable, omit it entirely.
5. Ensure the entire comment fits on a single line.

## 4. Verification Process

- Test each remote with the actual device when possible
- Use a consistent testing methodology for each device type
- Document any inconsistencies or special behaviors
- Implement a peer review system for submitted codes
- Use a version control system to track changes and allow rollbacks

## 5. Expanded Button Set

- Include device-specific buttons (e.g., Smart TV functions, Sound modes)
- Document any non-standard buttons in the metadata
- Group related buttons (e.g., numeric keypad, color buttons)

## 6. Integration with External Resources

- Reference official remote layouts when available
- Link to product manuals or specifications in metadata
- Consider creating a central database of common remote layouts

## 7. Automated Processing

Develop scripts for:
- Validating file structure and naming conventions
- Comparing new submissions against existing database
- Generating statistics on database coverage and completeness
- Automated testing with IR blasters and receivers
...
2. Capture all available buttons
3. Test each button multiple times
4. Use standardized button names
5. Include comprehensive metadata
6. Test file with Flipper Zero before submission
7. Document any special features or behaviors
